...
export default function Card({
  ...
  const imgElements = useMemo(() => {
    ...
    const sum = images.reduce((accumulator, { width }) => accumulator + width, 0);
    const dif =
      images.length > 1 && contentWidth < sum ? (sum - contentWidth) / (images.length - 1) : 0;
      ...
      const element = (
        <motion.div
          ...
          style={{
            ...
            display: index > 0 && images[0].width > contentWidth ? 'none' : 'block',
          }}
          variants={{
            hovered: { scale: 1.05 },
            pressed: { scale: 0.95 },
    ...
    >
      ...
      <div className="relative flex h-full flex-col px-11 pt-7">
        <div className="z-[2] mb-7">
          <h3 className="mb-1 text-xl font-bold text-black dark:text-white">{title}</h3>
      ...
      >
        <Light color={color} className="mt-[-60px]" blur={120} opacity={0.4} radius={100} />
      </div>
    </motion.a>
  );
}
